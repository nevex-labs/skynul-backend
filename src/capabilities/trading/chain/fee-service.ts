...
async function getTreasuryAddress(): Promise<string> {
  ...
  const addr = await getSecret('CHAIN_TREASURY_ADDRESS');
  if (!addr) {
...
async function canCollectFee(chainId: number, additionalAmount?: string): Promise<boolean> {
  try {
    const wallet = await EvmWallet.load();
    if (!wallet) return false;
    const usdcBalance = await wallet.getUsdcBalance(chainId);
    const feeAmount = Number.parseFloat(FEE_USDC);
    const extra = additionalAmount ? Number.parseFloat(additionalAmount) : 0;
    const total = feeAmount + extra;
    return Number.parseFloat(usdcBalance.balance) >= total;
  } catch {
    return false;
  }
}

async function collectFee(chainId: number): Promise<TxReceipt> {
  const wallet = await EvmWallet.load();
  if (!wallet) {
  ...
  const canPay = await canCollectFee(chainId);
  if (!canPay) {
    throw new Error(`Insufficient USDC balance for fee. Need at least ${FEE_USDC} USDC on ${chain.name}.`);
  }
...
function deductFeeFromAmount(grossAmount: number): number {
  return Math.max(0, grossAmount - Number.parseFloat(FEE_USDC));
}
