...
const BORDER_COLOR = { argb: 'FFD0D0D0' };
const CELL_BORDER = {
  top: { style: THIN_BORDER, color: BORDER_COLOR },
  bottom: { style: THIN_BORDER, color: BORDER_COLOR },
  left: { style: THIN_BORDER, color: BORDER_COLOR },
  right: { style: THIN_BORDER, color: BORDER_COLOR },
};

function applyRowStyling(sheet: ExcelJS.Worksheet, rowCount: number): void {
  ...
  for (let r = 1; r <= lastRow; r++) {
    for (let c = 1; c <= lastCol; c++) {
      sheet.getCell(r, c).border = CELL_BORDER;
    }
  }
}

function autoFitColumns(sheet: ExcelJS.Worksheet, headers: string[], rows: string[][]): void {
 ...
 */
export async function createExcelFromTsv(tsvData: string, filename: string, filter?: string): Promise<string> {
  ...
  applyRowStyling(sheet, rows.length);
