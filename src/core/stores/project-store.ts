...
export function projectList(): ProjectWithTasks[] {
  const d = getDb();
  const rows = d.prepare('SELECT * FROM projects ORDER BY created_at DESC').all() as Record<string, unknown>[];
  return rows.map((r) => {
    const tasks = d
...
export function projectCreate(name: string, color = '#6366f1'): ProjectWithTasks {
  ...
  const now = Date.now();
  getDb().prepare('INSERT INTO projects (id, name, color, created_at) VALUES (?, ?, ?, ?)').run(id, name, color, now);
...
export function projectRemoveTask(projectId: string, taskId: string): void {
  getDb().prepare('DELETE FROM project_tasks WHERE project_id = ? AND task_id = ?').run(projectId, taskId);
}
