...
type ContentPart = { type: 'text'; text: string } | { type: 'image'; mediaType: string; base64: string };
...
export async function deepseekVisionRespond(opts: {
  systemPrompt: string;
  ...
  for (let attempt = 0; attempt < MAX_RETRIES; attempt++) {
    res = await fetch(`${SUPABASE_URL}/functions/v1/chat-deepseek`, {
      method: 'POST',
      headers: {
  ...
  if (!content.trim()) throw new Error('Deepseek vision returned an empty response');
  return content;
}
