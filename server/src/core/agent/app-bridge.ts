...
const apps: Record<string, AppConfig> = {
  illustrator: {
    ...
    run: {
      darwin: (s) =>
        `osascript -e 'tell application "Adobe Illustrator" to do javascript file "${s}"'`
    }
  },
  photoshop: {
    scriptLang: 'extendscript',
    ...
    run: {
      darwin: (s) =>
        `osascript -e 'tell application "Adobe Photoshop 2024" to do javascript file "${s}"'`
    }
  },
  aftereffects: {
    scriptLang: 'extendscript',
    ...
    run: {
      darwin: (s) =>
        `osascript -e 'tell application "Adobe After Effects 2024" to do script file "${s}"'`
    }
  },
  blender: {
    scriptLang: 'python',
  ...
  async run(appName: AppName, script: string): Promise<AppBridgeResult> {
    ...
    try {
      ...
      if (isWSL && config.comName) {
        ...
        cmd = `powershell.exe -NoProfile -ExecutionPolicy Bypass -File "${winPs1Path}"`
      } else if (isWSL && config.run.win32) {
        // Windows via WSL: non-COM apps (Blender, Unreal)
        const winScriptPath = `C:\\Temp\\${scriptFileName}`
      ...
      // After executing, get a quick document inventory (Adobe ExtendScript only)
      if (config.scriptLang === 'extendscript' && config.comName) {
        try {
          const inventory = await this.getDocInventory(tmpDir, isWSL, config.comName, filesToClean)
      ...
      for (const f of filesToClean) await unlink(f).catch(() => {})
    }
  }
