...
function stripTrailingNoise(text: string): string {
  ...
  while (lines.length > 0) {
    ...
    if (
      !looksLikeJson &&
      (last.startsWith("·") ||
        /Starting agent loop|bridge ready|CDP browser/i.test(last))
    ) {
      lines.pop();
      continue;
    }
...
export function parseModelResponse(raw: string): ModelResponse {
  ...
  if (/^\s*\{\s*"thought"\s*:/.test(trimmed)) {
    ...
    if (consecutiveTruncations > MAX_TRUNCATION_RETRIES) {
      ...
      return {
        ...
        action: {
          type: "fail",
          reason:
            "Model output repeatedly truncated. Try a simpler task or shorter prompt.",
    ...
    const partialThought =
      trimmed.match(/"thought"\s*:\s*"([\s\S]{0,200})/)?.[1] ?? "";
    return {
